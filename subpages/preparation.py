...
from utils.helper import Timer, article_txt_reader

empty_messages: empty = empty()
empty_articles: empty = empty()

pre_sessions: list[str] = ["raw", "pTimer"]
    ...
    if session_state["raw"] is None:
        ...
        if button("Load the Raw Text Data", type="primary", width="stretch"):
            with spinner("Preparing the text data...", show_time=True, width="stretch"):
                with Timer("Load the Raw Text Data") as session_state["pTimer"]:
                    session_state["raw"]: str = article_txt_reader(TEXT_FILE_PATH)
            rerun()
    else:
        empty_messages.success(f"{session_state["pTimer"]} Text data is prepared. You can proceed to the next step.")
